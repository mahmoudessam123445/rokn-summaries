...
// ===== Load Summaries with Department Filter =====
let allSummaries = [];
let currentFilter = 'preparatory';
...
function updateDeptCounts() {
    const counts = { preparatory: 0, communications: 0, civil: 0, chemistry: 0, general: 0 };
...
function renderSummaries() {
    ...
    let filtered = allSummaries.filter(s => (s.department || 'general') === currentFilter);
...
async function loadSummaries() {
    ...
    try {
        ...
        // Auto-select first department that has summaries
        const deptOrder = ['preparatory', 'communications', 'civil', 'chemistry', 'general'];
        let foundDept = null;

        for (const dept of deptOrder) {
            const hasSummaries = allSummaries.some(s => (s.department || 'general') === dept);
            if (hasSummaries) {
                foundDept = dept;
                break;
            }
        ...
        if (foundDept) {
            currentFilter = foundDept;
            document.querySelectorAll('.dept-filter-btn').forEach(btn => {
                btn.classList.remove('active');
                if (btn.dataset.dept === foundDept) {
                    btn.classList.add('active');
                }
            });
        }
